...
[LagrangeSylvesterInterpolation](https://github.com/singularitti/LagrangeSylvesterInterpolation.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("LagrangeSylvesterInterpolation")
   ```

3. Run

   ```julia-repl
   julia> using LagrangeSylvesterInterpolation
   ```
...
version of LagrangeSylvesterInterpolation, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/LagrangeSylvesterInterpolation.jl"
```
...
our [GitHub repository](https://github.com/singularitti/LagrangeSylvesterInterpolation.jl)
for new releases.
Once we release a new version, you can update LagrangeSylvesterInterpolation by typing

```@repl
using Pkg
Pkg.update("LagrangeSylvesterInterpolation")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("LagrangeSylvesterInterpolation")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall LagrangeSylvesterInterpolation.
