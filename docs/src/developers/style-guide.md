...
!!! info
    The style guide is always a work in progress, and not all LagrangeSylvesterInterpolation code
    follows the rules. When modifying LagrangeSylvesterInterpolation, please fix the style violations
...
LagrangeSylvesterInterpolation uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/LagrangeSylvesterInterpolation.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the LagrangeSylvesterInterpolation directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to LagrangeSylvesterInterpolation have
